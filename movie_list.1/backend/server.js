...
const port = 3001; // grab the port variable from the environment to use later
...
// ========== Routes ==========
//I should be making routes to my front end again apparently??? 
// the url endpoint is like the home of where the route lives so that it can be called by the front end JS for an event listener.
...
app.post("/get-movie-by-name", (req, res) => {
  const searchTerm = req.body.name;
  pool
    .query(`SELECT * FROM movies WHERE movie_name = '${searchTerm}'`)
    .then((result) => {
      res.send(result.rows[0]);
    })
    .catch((err) => {
...
//create a route for posting new movies to the database.
app.post("/movies", (req, res) => {
  let { movie_name, released_date, run_time, rating} = req.body
  pool
    .query(
